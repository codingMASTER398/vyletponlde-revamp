...
function convertToAcronyms(text) {
  ...
  const acronymSimplified = makeAcronym(text.replace(/[^a-zA-Z 0-9]/g,'').replaceAll("is", "").replaceAll("a", "").replaceAll("and", "").replaceAll("the", "").replaceAll("of", ""))

  if(acronym.length > 1 && acronym != text) out.push(acronym);
  if(phoneticLatin.length > 1 && phoneticLatin != text) out.push(phoneticLatin);
  if(greeklish.length > 1 && greeklish != text) out.push(greeklish);
  if(acronymSimplified.length > 1 && acronymSimplified != text) out.push(acronymSimplified);
